...
export const fileSchema = (max_size?: number, file_types?: MimeTypes[]) => {
  const schema = z
    .file()
    .max(max_size ?? MAX_FILE_SIZE)
    .mime([...(file_types ?? ACCEPTED_IMAGE_TYPES)]);
...
export const fileArraySchema = (
  ...
  const schema = z
    ...
    .min(min_amount ?? MIN_ARRAY_AMOUNT)
    .max(max_amount ?? MAX_ARRAY_AMOUNT);
